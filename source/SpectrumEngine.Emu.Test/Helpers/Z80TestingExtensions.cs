...
/// <summary>
/// This class provides extension methods for testing the Z80 CPU and related classes.
/// </summary>
public static class Z80TestingExtensions
{
    ...
    public static void ShouldKeepRegisters(this Z80TestMachine machine, string? except = null)
    {
        ...
        var after = machine.Cpu.Regs;
        var exclude = except?.Split(',') ?? Array.Empty<string>();
        ...
        throw new Exception("The following registers are expected to remain intact, " +
    ...
    {
        var before = (machine.RegistersBeforeRun!.F & Z80Cpu.FlagsSetMask.S) != 0;
        var after = (machine.Cpu.Regs.F & Z80Cpu.FlagsSetMask.S) != 0;
        if (after == before)
        {
            return;
        }
        throw new Exception($"S flag expected to keep its value, but it changed from {before} to {after}");
    }
    ...
    {
        var before = (machine.RegistersBeforeRun!.F & Z80Cpu.FlagsSetMask.Z) != 0;
        var after = (machine.Cpu.Regs.F & Z80Cpu.FlagsSetMask.Z) != 0;
        if (after == before)
        {
            return;
        }
        throw new Exception($"Z flag expected to keep its value, but it changed from {before} to {after}");
    }
    ...
    {
        var before = (machine.RegistersBeforeRun!.F & Z80Cpu.FlagsSetMask.N) != 0;
        var after = (machine.Cpu.Regs.F & Z80Cpu.FlagsSetMask.N) != 0;
        if (after == before)
        {
            return;
        }
        throw new Exception($"N flag expected to keep its value, but it changed from {before} to {after}");
    }
    ...
    {
        var before = (machine.RegistersBeforeRun!.F & Z80Cpu.FlagsSetMask.PV) != 0;
        var after = (machine.Cpu.Regs.F & Z80Cpu.FlagsSetMask.PV) != 0;
        if (after == before)
        {
            return;
        }
        throw new Exception($"PV flag expected to keep its value, but it changed from {before} to {after}");
    }
    ...
    {
        var before = (machine.RegistersBeforeRun!.F & Z80Cpu.FlagsSetMask.H) != 0;
        var after = (machine.Cpu.Regs.F & Z80Cpu.FlagsSetMask.H) != 0;
        if (after == before)
        {
            return;
        }
    ...
    {
        var before = (machine.RegistersBeforeRun!.F & Z80Cpu.FlagsSetMask.C) != 0;
        var after = (machine.Cpu.Regs.F & Z80Cpu.FlagsSetMask.C) != 0;
        if (after == before)
        {
            return;
        }
        throw new Exception($"C flag expected to keep its value, but it changed from {before} to {after}");
    }
    ...
    public static void ShouldKeepMemory(this Z80TestMachine machine, string? except = null)
    {
        ...
        // --- Parse ranges
        var strRanges = except?.Split(',') ?? Array.Empty<string>();
        ...
        var upperMemoryBound = (int)machine.Cpu.Regs.SP;
        ...
        {
            throw new Exception("The following memory locations are expected to remain intact, " +
