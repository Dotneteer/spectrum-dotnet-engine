...
{
    private readonly List<DisassemblyItem> _outputItems = new();

    private readonly Dictionary<ushort, DisassemblyItem> _outputByAddress = new();

    private readonly Dictionary<ushort, DisassemblyLabel?> _labels = new();
    ...
    public DisassemblyItem? this[ushort addr]
    {
        get
        {
            _outputByAddress.TryGetValue(addr, out var item);
            return item;
        }
    }
